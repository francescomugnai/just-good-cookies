...
class JustGoodCookies {
    ...
    if(preference['duration']){
      ...
      switch (getValue) {
        ...
        case "1": // Cookies accepted :)
          bannerContent.classList.add(this.checkTailwindPrefix('hidden')); 
          // Check if we are running the autoMode
          this.checkCookiesAutoMode()
          // Remove placeholders
          this.removePlaceholders()
          // Remove hidden divs (if any) for accepted cookies
          this.removeDivsOfUserAcceptedIframes()
          // Check if the cookie is expired
          this.checkCookieExpiration() 
          // Check if we need to activate some pre-built scripts
          this.checkActivations() 
          // Check Google Analytics
          this.checkGoogleAnalytics()
          // We enable cookies and manage them through the settings panel
          this.activateToggledCookies() 
          // Check if we need to turn on Google Tag Manager
  ...
  */
  googleTagManager(){
    if(this.activate?.GoogleTagManager){
      let dataObject = { 'event': this.activate.GoogleTagManager.event_name };
  ...
  */
  returnServices(service){
    ...
    if(this.activate){
      const objEntries = Object.entries(this.activate)
      for (let i = 0; i < objEntries.length; i++) {
        const element = objEntries[i];
        if(element[1].dataJgcService && service == element[1].dataJgcTag){
          arr+= this.makeArrForServices(element[1].dataJgcService)
        }
      }
    }
  ...
  */
  openPanel(){
    ...
    if(refreshed == null && (this.panel && this.panel.open)) {
        ...
        if(this.config.layout == 'style8'){ // Style8 is a bit particular...
          banner.remove()
          setTimeout(() => {
  ...
  */
  animateToggle(val, buttonType){
    ...
    if(val && (toggle != null && toggleRight != null)){
      ...
      toggleRight.classList.add(this.checkTailwindPrefix('border-green-400'))
      // toggleRight.classList.add(this.checkTailwindPrefix('translate-x-full'))
      ...
      toggleNecessary.classList.add(this.checkTailwindPrefix('border-green-400'))
      // toggleNecessary.classList.add(this.checkTailwindPrefix('translate-x-full'))
  ...
  */
  closeBanner() {
    ...
    if(banner){
      ...
      if(this.config.layout != 'style8'){ // "style8" is a little special. I can not use animations here because the toggle switches repeat in 2 different areas.
        setTimeout(() => {
  ...
  */
  loadBannerLayout(style){
    ...
    case "style7":
        ...
        this.banner.style.display = "none";
        this.banner.innerHTML = `
        <div id="bannerContent" style="${ this.bannerConfig.backgroundImage ? 
              `background-size:cover; background-image:url(${ this.bannerConfig.backgroundImage })` : ''}" 
              class="
              ${ this.positions[this.bannerConfig.position || 'bottom'] }
              ${ this.bannerConfig.backgroundColor } 
              ${ this.bannerConfig.backgroundImage ? `${ this.bannerConfig.backgroundColor } ${this.checkTailwindPrefix('p-2')}` : `${ this.bannerConfig.innerBackgroundImage ? '' : this.checkTailwindPrefix('p-6') }` }
